...
public class ElevSubSys extends Subsystem {
    ...
    static private double kTopVoltDefault = 4.367;			//Voltage at top    //THESE DON'T UPDATE IN CODE
    static private double kBotVoltDefault = 0.875;			//Voltage at bottom
    static private double kTopVolt, kBotVolt ;

    static final private double ELEV_TOTAL_DIST = 39.125;	//Distance in inches covered by Pot.
    static private double kVoltRange= 3.492;
    static private double ELEV_POT_CONV = ((kTopVoltDefault - kTopVoltDefault) / ELEV_TOTAL_DIST); //Volts per inch
...
//   	public static final double KHoldSpeedDefault  = +0.18;		// Practice bot
    ...
    public static final double KLimitElevTopPos= 35;
    ...
    public void elevMoveMtr(double speed){
    	...
    	if(mCurrElevPwr > KHoldSpeed) {
    		...
    		if((getElevPos()>= KLimitElevTopPos) && (mCurrElevPwr>= KRaiseSlowSpeed)) {
    	...
    	if(mCurrElevPwr < KHoldSpeed) {
    		...
    		if((getElevPos()<= KLimitElevBotPos) && (mCurrElevPwr <= KLowerSlowSpeed)) {
    ...
    public void recalibrateVoltages(){
    	...
    	ELEV_POT_CONV = ((kBotVolt - kTopVolt) / ELEV_TOTAL_DIST); //Volts/degree
    	double pos = getElevPos();
    	if (mPrintFlag == 0) {
    		mPrintFlag = 1; 		// lets not immediately reprint values
    		double voltDiff = kBotVolt - kTopVolt;
    		mLastPos = pos;
    		line += (" kTopVolt=" + kTopVolt + " kBotVolt=" + kBotVolt + " voltDiff=" + voltDiff + " ElevPos=" + pos);
    ...
    public void getElevPrefs() {
    	...
    	// remove next 2 lines afet getting these into prefs
    	...
    	kBotVolt = getPot();					// Robot Elev starts at bottom so calibrate at this time
    	kTopVolt = kBotVolt - kVoltRange ;
    	ELEV_POT_CONV = ((kBotVolt - kTopVolt) / ELEV_TOTAL_DIST); //Volts per inch
   ...
   // a Command can call this to force a stop (e.g., from a watchdog timer)    
	...
	public void updateSmartDash() {
     	...
     	SmartDashboard.putNumber("elev CurrVolt", getPot()) ;     	
